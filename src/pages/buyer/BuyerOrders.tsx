    ...
    <div className="space-y-6">
      ...
      {orders.length === 0 ? (
        <div className="bg-white p-12 text-center rounded-lg shadow-sm border border-slate-200">
          ...
          <div className="mt-6">
            <Link to="/products" className="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-green-600 hover:bg-green-700">
              Start Shopping
            </Link>
          </div>
        </div>
            ...
            {orders.map((order) => (
              <li key={order.id} className="p-4 hover:bg-slate-50 transition-colors">
                <div className="flex items-center justify-between">
                  ...
                  <div>
                    <Link to={`/buyer/orders/${order.id}`} className="text-green-600 hover:text-green-700 p-2 inline-flex items-center text-sm font-medium">
                      View Details
