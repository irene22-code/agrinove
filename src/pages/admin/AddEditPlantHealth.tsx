...
export default function AddEditPlantHealth() {
    ...
    const [lookups, setLookups] = useState<any>({ types: [], categories: [], crops: [], parts: [], spreadMethods: [], riskLevels: [], seasons: [], causeTypes: [] });
    ...
    const handleRiskChange = (e: any) => setFormData((prev: any) => ({ ...prev, risk_conditions: { ...prev.risk_conditions, [e.target.name]: e.target.value } }));
    const handleArrayChange = (name: string, values: string[]) => setFormData((prev: any) => ({ ...prev, [name]: values }));
    ...
    const addListItem = (field: string, item: any) => setFormData((prev: any) => ({ ...prev, [field]: [...(prev[field] || []), item] }));
    const updateListItem = (field: string, index: number, key: string, value: any) => setFormData((prev: any) => { const newArr = [...(prev[field]||[])]; if(newArr[index]) newArr[index] = {...newArr[index], [key]: value}; return { ...prev, [field]: newArr }; });
    const removeListItem = (field: string, index: number) => setFormData((prev: any) => { const newArr = [...(prev[field]||[])]; newArr.splice(index, 1); return { ...prev, [field]: newArr }; });
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select name="risk_level" value={formData.risk_level} onChange={handleChange} className="w-full border rounded-lg p-2">
                                ...
                                {lookups.riskLevels?.map((r: any) => <option key={r.id} value={r.name}>{r.name}</option>)}
                            </select>
                        </div>
                    </div>
                </section>
                                ...
                                <div className="flex-1 space-y-3">
                                    ...
                                    <div className="flex gap-4 items-center mt-2">
                                        ...
                                        <div className="relative overflow-hidden inline-block">
                                            ...
                                            <input type="file" title=" " onChange={(e) => handleFileUpload(e, 'image', (url) => { updateListItem('symptoms', idx, 'image_url', url);  })} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
                                        </div>
                                    </div>
                            ...
                            <div key={idx} className="flex gap-4 items-start bg-slate-50 p-4 rounded-lg border">
                                <div className="flex-1 space-y-3">
                                    ...
                                    <div className="flex gap-4 items-center">
                                        <input type="text" placeholder="Thumbnail URL (Optional)" value={v.thumbnail_url || ''} onChange={e => updateListItem('videos', idx, 'thumbnail_url', e.target.value)} className="flex-1 border rounded p-2" />
                                        <span className="text-sm font-medium">OR</span>
                                        <div className="relative overflow-hidden inline-block">
                                            <button className="bg-slate-200 px-3 py-2 rounded text-sm font-medium">Upload Thumbnail</button>
                                            <input type="file" accept="image/*" title=" " onChange={(e) => handleFileUpload(e, 'image', (url) => { updateListItem('videos', idx, 'thumbnail_url', url);  })} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div key={idx} className="flex gap-4 items-start bg-slate-50 p-4 rounded-lg border">
                                <div className="flex-1 space-y-3">
                                    ...
                                    <div className="flex gap-4 items-center">
                                        ...
                                        <div className="relative overflow-hidden inline-block">
                                            <button className="bg-slate-200 px-3 py-2 rounded text-sm font-medium">Upload File</button>
                                            <input type="file" title=" " onChange={(e) => handleFileUpload(e, 'document', (url) => { updateListItem('resources', idx, 'file_url', url);  })} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
                                        </div>
                                    </div>
                                </div>
