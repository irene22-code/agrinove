            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-8 relative z-10">
                <div className="bg-white rounded-xl shadow-lg p-4 mb-8 grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div className="relative md:col-span-1">
                        ...
                        <input type="text" name="search" value={filters.search} onChange={handleFilterChange} placeholder="Search problems..." className="w-full pl-10 pr-4 py-3 border border-slate-200 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 bg-slate-50" />
                    </div>
                    <select name="crop_id" value={filters.crop_id} onChange={handleFilterChange} className="border border-slate-200 rounded-lg px-4 py-3 bg-slate-50 focus:ring-2 focus:ring-green-500">
                        <option value="">All Crops</option>
                        {lookups.crops.map((c: any) => <option key={c.id} value={c.id}>{c.name}</option>)}
                    </select>
                    <select name="type_id" value={filters.type_id} onChange={handleFilterChange} className="border border-slate-200 rounded-lg px-4 py-3 bg-slate-50 focus:ring-2 focus:ring-green-500">
                        <option value="">All Types</option>
                        {lookups.types.map((t: any) => <option key={t.id} value={t.id}>{t.name}</option>)}
                    </select>
                    <select name="risk_level" value={filters.risk_level} onChange={handleFilterChange} className="border border-slate-200 rounded-lg px-4 py-3 bg-slate-50 focus:ring-2 focus:ring-green-500">
                ...
                <div className="mb-6 flex justify-between items-center">
                    ...
                    {(filters.search || filters.crop_id || filters.type_id || filters.risk_level) && (
                        <button onClick={clearFilters} className="text-sm font-medium text-green-600 hover:text-green-700">Clear Filters</button>
                    )}
                </div>

                {loading ? (
                    <div className="flex justify-center py-20"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-green-600"></div></div>
                ) : problems.length > 0 ? (
                            ...
                            <Link key={problem.id} to={`/plant-health/${problem.slug}`} className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden hover:shadow-md transition-all hover:-translate-y-1 group flex flex-col h-full">
                                <div className="aspect-[4/3] bg-slate-100 relative overflow-hidden">
                                    ...
                                    <div className="absolute top-3 right-3">
                                        <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold shadow-sm ${problem.risk_level === 'High' || problem.risk_level === 'Critical' ? 'bg-red-500 text-white' : problem.risk_level === 'Medium' ? 'bg-amber-500 text-white' : 'bg-green-500 text-white'}`}>
                                            {problem.risk_level} Risk
                                        </span>
                                    </div>
                                </div>
                                <div className="p-5 flex flex-col flex-1">
                                    <div className="text-xs font-bold text-green-600 uppercase tracking-wider mb-1 line-clamp-1">{problem.plant_health_problem_types?.name} {problem.cause_type && `• ${problem.cause_type}`}</div>
                                    <h3 className="text-lg font-bold text-slate-900 mb-1 line-clamp-1 group-hover:text-green-700 transition-colors">{problem.name}</h3>
                                    ...
                                    <div className="mt-4 flex items-center text-sm font-bold text-green-600 group-hover:text-green-700 transition-colors">
                    ...
                    <div className="text-center py-20 bg-white rounded-xl shadow-sm border border-slate-200">
                        ...
                        <button onClick={clearFilters} className="bg-green-100 text-green-700 px-6 py-2 rounded-lg font-medium hover:bg-green-200 transition-colors">Clear All Filters</button>
                    </div>
                )}
            </div>
        </div>
