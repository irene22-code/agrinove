...
buyers = buyers.replace(
  ...
  `const handleDelete = async (id: string) => {
    console.log("Delete clicked in AdminBuyers");
    console.log("Buyer id:", id);`
);
buyers = buyers.replace(
  /const res = await api.delete/,
  `console.log("Sending delete request to API for buyer", id);
      const res = await api.delete`
);
...
sellers = sellers.replace(
  /const handleDelete = async \(id: string\) => {/,
  `const handleDelete = async (id: string) => {
    console.log("Delete clicked in AdminSellers");
    console.log("Seller id:", id);`
);
sellers = sellers.replace(
  /const res = await api.delete/,
  `console.log("Sending delete request to API for seller", id);
      const res = await api.delete`
);
...
users = users.replace(
  ...
  `const handleDeleteUser = async (id: string) => {
    console.log("Delete clicked in AdminUsers");
...
routes = routes.replace(
  ...
  console.log("Admin route reached: DELETE /users/:id", req.params.id);
  next();
}, deleteUser);`
);
