        ...
        <div className="bg-slate-50 min-h-screen pb-20">
            ...
            <div className="bg-white border-b">
                <div className="max-w-4xl mx-auto px-4 py-6">
                    <Link to="/plant-health" className="inline-flex items-center text-sm font-medium text-emerald-600 hover:text-emerald-700 mb-6">
                    ...
                    <div className="flex flex-col md:flex-row gap-8 items-start">
                        ...
                        <div className="flex-1">
                            <div className="flex items-center gap-3 mb-2">
                                ...
                                <span className={\`px-3 py-1 rounded-full text-sm font-bold \${problem.risk_level === 'High' || problem.risk_level === 'Critical' ? 'bg-red-100 text-red-800' : 'bg-emerald-100 text-emerald-800'}\`}>{problem.risk_level} Risk</span>
                            </div>
            ...
            <div className="max-w-4xl mx-auto px-4 py-8 space-y-8">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                            {additionalImages.map((img:any, i:number) => (
                                <div key={i} className="group relative">
                                    ...
                                    <div className="mt-2">
                                        <span className="text-xs font-bold text-emerald-600 uppercase">{img.image_type}</span>
                ...
                <section className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 md:p-8">
                    <h2 className="text-xl font-bold text-slate-900 mb-4 flex items-center gap-2"><Info className="h-5 w-5 text-emerald-600" /> About This Problem</h2>
                ...
                {problem.plant_health_symptoms?.length > 0 && (
                    <section className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 md:p-8">
                        <h2 className="text-xl font-bold text-slate-900 mb-6 flex items-center gap-2"><Activity className="h-5 w-5 text-emerald-600" /> Symptoms</h2>
                ...
                {(problem.plant_health_problem_spread?.length > 0 || riskConditions) && (
                    <section className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 md:p-8">
                        <h2 className="text-xl font-bold text-slate-900 mb-6 flex items-center gap-2"><Wind className="h-5 w-5 text-emerald-600" /> Spread & Risk Conditions</h2>
                        
                        {problem.plant_health_problem_spread?.length > 0 && (
                            <div className="mb-6">
                                <h3 className="text-lg font-bold text-slate-800 mb-2">How it Spreads</h3>
                                <div className="flex flex-wrap gap-2 mb-3">
                                    {problem.plant_health_problem_spread.map((s:any) => (
                                        <span key={s.spread_method_id} className="bg-blue-50 text-blue-700 border border-blue-200 px-3 py-1 rounded-full text-sm">{s.plant_health_spread_methods?.name}</span>
                                    ))}
                                </div>
                ...
                {(problem.plant_health_prevention?.length > 0 || problem.plant_health_control_methods?.length > 0) && (
                    <section className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 md:p-8">
                        <h2 className="text-xl font-bold text-slate-900 mb-6 flex items-center gap-2"><ShieldCheck className="h-5 w-5 text-emerald-600" /> Management & Control</h2>
                                ...
                                <ul className="space-y-3">
                                    {problem.plant_health_prevention.map((p:any) => (
                                        <li key={p.id} className="flex gap-3 text-slate-700">
                                            <div className="h-6 w-6 rounded-full bg-emerald-100 text-emerald-600 flex items-center justify-center shrink-0 font-bold text-xs"><CheckCircle2 className="h-4 w-4" /></div>
                                            <div>
                            ...
                            <div>
                                ...
                                <div className="space-y-4">
                                    {problem.plant_health_control_methods.map((c:any) => (
                                        <div key={c.id} className="bg-slate-50 border-l-4 border-emerald-500 p-4 rounded-r-lg">
                                            <div className="text-xs font-bold text-emerald-600 uppercase tracking-wider mb-1">{c.control_type}</div>
                ...
                {problem.plant_health_videos?.length > 0 && (
                    <section className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 md:p-8">
                        <h2 className="text-xl font-bold text-slate-900 mb-6 flex items-center gap-2"><Video className="h-5 w-5 text-emerald-600" /> Educational Videos</h2>
                            ...
                            {problem.plant_health_videos.map((v:any, idx:number) => (
                                <a key={idx} href={v.video_url} target="_blank" rel="noopener noreferrer" className="block group border rounded-xl overflow-hidden hover:shadow-md transition-all">
                                    <div className="aspect-video bg-slate-900 relative flex items-center justify-center">
                                        ...
                                        <div className="absolute inset-0 flex items-center justify-center"><div className="w-12 h-12 bg-white/90 rounded-full flex items-center justify-center"><div className="w-0 h-0 border-t-8 border-t-transparent border-l-[14px] border-l-emerald-600 border-b-8 border-b-transparent ml-1"></div></div></div>
                                    </div>
                                    <div className="p-4 bg-white">
                                        <h4 className="font-bold text-slate-900 mb-1 group-hover:text-emerald-600">{v.title}</h4>
                ...
                {problem.plant_health_resources?.length > 0 && (
                    <section className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 md:p-8">
                        <h2 className="text-xl font-bold text-slate-900 mb-6 flex items-center gap-2"><FileText className="h-5 w-5 text-emerald-600" /> Farmer Guide & Resources</h2>
                        <div className="space-y-3">
                            {problem.plant_health_resources.map((r:any, idx:number) => (
                                <a key={idx} href={r.file_url} target="_blank" rel="noopener noreferrer" className="flex items-start gap-4 p-4 rounded-xl border border-slate-200 hover:border-emerald-300 hover:bg-emerald-50 transition-colors group">
                                    <div className="h-10 w-10 bg-emerald-100 text-emerald-600 rounded-lg flex items-center justify-center shrink-0 group-hover:bg-emerald-200"><FileText className="h-5 w-5" /></div>
                                    <div className="flex-1">
                                        <h4 className="font-bold text-slate-900 mb-1 group-hover:text-emerald-700">{r.title}</h4>
                                        <p className="text-sm text-slate-600">{r.description}</p>
                                        <div className="mt-2 text-xs font-bold text-emerald-600 uppercase flex items-center gap-1"><ExternalLink className="h-3 w-3" /> {r.resource_type}</div>
                                    </div>
                                </a>
                            ))}
                        </div>
                ...
                {problem.plant_health_expert_advice?.length > 0 && (
                    <section className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 md:p-8">
                        <h2 className="text-xl font-bold text-slate-900 mb-6 flex items-center gap-2"><Bookmark className="h-5 w-5 text-emerald-600" /> Expert Advice</h2>
                        <div className="space-y-4">
                        ...
                        <ul className="space-y-3">
                            {problem.plant_health_sources.map((s:any, idx:number) => (
                                <li key={idx} className="text-sm text-slate-600">
                                    ...
                                    {s.source_url && <a href={s.source_url} target="_blank" rel="noopener noreferrer" className="ml-2 text-emerald-600 hover:underline">Link</a>}
...
`;
fs.writeFileSync('src/pages/public/PlantHealthDetails.tsx', content);
